...
`core/spec.md` Step 6「`.claude/rules/` 生成」の仕様に従い、プロジェクト規約ファイルを生成する。

```
# 1. 既存チェック（存在する場合はスキップ）
Glob(".claude/rules/")
→ 存在する場合: このステップ全体をスキップ

# 2. config.yaml から tech_stack を読み取る（Step 1 で読み込み済み）
Read(".blueprint/config.yaml")

# 3. base.md は常に生成
Read("skills/spec/references/claude-rules-templates/base.md")
Write(".claude/rules/base.md", {content})

# 4. TypeScript プロジェクトの場合
if config.tech_stack.language === "typescript":
  Read("skills/spec/references/claude-rules-templates/typescript.md")
  Write(".claude/rules/typescript.md", {content})

# 5. React / Next.js プロジェクトの場合
if config.tech_stack.frontend.framework in ["react", "next"]:
  Read("skills/spec/references/claude-rules-templates/react.md")
  Write(".claude/rules/react.md", {content})

# 6. @testing-library プロジェクトの場合
if config.tech_stack.frontend.test_tool === "testing-library":
  Read("skills/spec/references/claude-rules-templates/testing-library.md")
  Write(".claude/rules/testing-library.md", {content})
```

> **注**: `frontend` セクションが config.yaml にない場合（バックエンドのみプロジェクト）は Step 5/6 をスキップ。
