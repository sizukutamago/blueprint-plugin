...
2. 共通メタデータの存在チェック:
   ...
   - type: 必須、api | external | file | internal | screen → 欠落/不正値: P0
   - subtype: type が internal の場合必須、service | repository → 欠落/不正値: P0
   - screen_type: type が screen の場合必須、form | list | detail | dashboard → 欠落/不正値: P0
...
4. タイプ別必須フィールド:
   ...
   screen:
   - screen_type: 必須 (form|list|detail|dashboard) → 欠落/不正値: P0
   - route: 必須（path + auth_required） → 欠落: P0
   - route.path が "/" で始まること → 違反: P1
   - links.depends_on が配列であること → 非配列: P1
   - [form] form.submit_action が存在すること → 欠落: P0
   - [form] form.fields が存在すること → 欠落: P0
   - [list] list.data_source が存在すること → 欠落: P0
   - [detail] detail.data_source が存在すること → 欠落: P0
   - [dashboard] dashboard.widgets が存在かつ 1 件以上 → 欠落/空配列: P0
...
2. links.depends_on 参照先チェック:
   ...
   → 実在しない: P1
   - 循環参照がないか → 検出: P0（循環依存は Stage 3 のトポロジカルソートを破壊する）
...
5. 入力フィールドの制約定義チェック:
   ...
   screen — validation_rules の各ルール（form の場合）:
   - id (VR-xxx) が存在するか → 未付与: P1
   - field が存在するか → 欠落: P1
   - constraint が存在するか → 欠落: P1（テスト導出不可）
   screen — form.fields の各フィールド:
   - required / max_length / pattern / enum のいずれかが存在するか → 全欠落: P2
...
3. business_rules / rules のテスト可能性:
   ...
   - screen: validation_rules[].constraint に具体的な条件値があるか → 抽象的: P1
   - screen: form.fields[].required/max_length/pattern/enum のいずれかがあるか → 全欠落: P1
