...
| 項目 | 方針 |
|------|------|
| 個人情報（PII）の取り扱い | テストデータに本番の個人情報を使用しない。ファクトリー関数で擬似データを生成する |
| データマスキング | 本番データをテスト環境に投入する場合は、PII フィールド（氏名、メール、電話番号等）を匿名化/マスキングしてから使用する |
| マスキング方式 | {{MASKING_METHOD}}（例: 不可逆ハッシュ、ランダム置換、仮名化） |
| テストデータの保持期限 | テストサイクル終了後 {{RETENTION_DAYS}} 日以内に削除 |
| テストデータ生成ツール | {{DATA_GEN_TOOL}}（例: Faker.js, Factory Bot, 独自シードスクリプト） |
| データ分類別の扱い | PII: 擬似データのみ / Sensitive: マスキング必須 / Internal: 本番コピー可 / Public: 制限なし |
