...
各エージェントに以下を渡す:
- Contract YAML ファイルパスリスト（`.knowledge/contracts/**/*.contract.yaml`）
...
2. 共通メタデータの存在チェック:
   ...
   - type: 必須、api | external | file → 欠落/不正値: P0
...
2. links.depends_on 参照先チェック:
   - 参照先 CON-xxx が .knowledge/contracts/ に実在するか
   → 実在しない: P1
   - 循環参照がないか → 検出: P1

3. links.decided_by チェック:
   - DEC-xxx が .knowledge/decisions/ に実在するか
   → 実在しない: P2（決定ログ連携は推奨）
...
3. business_rules のテスト可能性:
   - 各 BR-xxx に正常系と異常系の判定基準が明確か
   → 不明確: P1（テストの期待値が定義できない）
