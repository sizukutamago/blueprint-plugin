...
export default defineConfig({
  ...
  webServer: [
    {
      command: 'npm run dev',          // package.json から適切なコマンドを検出
      url: 'http://localhost:3000',
      reuseExistingServer: !process.env.CI,
    },
    {
      command: 'npm run dev:client',   // Vite 起動コマンドを検出
...
```typescript
// @generated by /test-from-contract
// @contract CON-{screen-name}
import { test, expect } from '@playwright/test'

test.describe('{Screen名}', () => {
  // --- smoke: 全 screen Contract に必須 ---
  ...
  // --- 品質保証: business_rules から導出 ---
  // BR-XXX: フォーム送信 → ページ遷移 → 結果反映
  test('BR-XXX: ...', async ({ page }) => {
    const unique = `テスト-${Date.now()}`
    // Derived from: business_rules[BR-XXX]
    // ...
  })
})
```
