...
4. tests/ui/ の RED テスト（screen Contract）を確認（存在する場合）
5. 既存コードの構造を把握（brownfield の場合）
```
...
**state_transition フィールドがある Contract の実装**:

```
実装手順:
1. transitions マップを domain 層の定数として配置する
...
2. canTransition(from, to) 関数を domain 層に定義する
   - from の transitions に to が含まれる場合のみ true

3. ルートハンドラで canTransition を呼び出す（ハードコードの条件分岐を避ける）
   if (data.status && !canTransition(sticky.status, data.status)) {
     return error(errors[] のエラーコード, 409)
   }

4. Refactorer フェーズで「domain 関数が route で使われているか」を確認する
   → インポートのみで未使用の場合はデッドコードとして修正必須
```

> **重要**: domain 層で定義した canTransition 等のビジネスロジック関数は
> ルートハンドラから必ず呼び出す。定義のみで呼び出さない場合は「Dead import」として
> Refactorer が Step 5 で必ず修正する。
...
| screen | ページコンポーネント、フォームバリデーション、API 呼び出し |

**screen Implementer の実装内容**:

```
screen Contract の場合:
1. `tests/ui/{screen-name}/` の RED テストを確認
2. `src/interface/{screen-name}/` 配下のみ編集（名前空間分離）
3. 生成ファイル:
   - {ScreenName}Page.tsx          # ページコンポーネント
   - {ScreenName}Page.test.tsx     # UI テスト（tests/ui/ からシンボリックリンクまたはコピー）
   - components/{ScreenName}Form.tsx 等
4. validation_rules → フロントエンドバリデーションロジックに変換
   - pattern → zod/yup スキーマ or native HTML5 validation
5. submit_action / data_source の Contract を参照してリクエスト形式を確認
6. config.yaml の tech_stack.frontend.framework に従って実装
   （react: tsx, vue: vue, next: tsx など）
```
     ...
     return (
       <BrowserRouter>
         <Routes>
           ...
           <Route path="/{screen-path}" element={<...Route />} />
         </Routes>
       </BrowserRouter>
     )
   }
...
2.5. 依存パッケージ整合性チェック
   - src/ 配下の全ファイルから import 文（静的・動的）を抽出
   - 外部パッケージ（node_modules）を列挙
   - package.json の dependencies + devDependencies と差分を比較
   - 差分がある場合: 自動でインストールして package.json を更新
     - 本番コード（src/）で使うパッケージ → dependencies
     - テストのみ（tests/）で使うパッケージ → devDependencies
   - `await import(...)` 等の動的インポートも対象に含める

3. 全テスト一括実行（Level 1 + Level 2 + Unit + UI テスト）
   - 失敗テストがあれば修正を試みる
   - 同じエラーが 3 回連続したらユーザーに報告
4. ブロックされた Contract のリスト提示
5. import 循環の検出（レイヤー違反チェック）
```
...
3. 構造改善:
   ...
   - レイヤー構造の整合性確認
4. デッドコード検出（必須）:
   - import されているが使われていない関数・型を検出
   - domain 層のビジネスロジック関数（canTransition 等）が
     usecase/route 層で呼ばれているかを確認
   - 定義のみで未使用 → 削除 or ルートで使うよう修正
   - ハードコードの条件分岐（if status === 'done'）が domain 関数で
     代替できる場合は domain 関数を使うよう統一
5. 全テスト実行（リファクタで壊れていないことを確認）
```
...
## Phase D: Web App 動作確認

### Step 6.5: ブラウザ動作確認

実装したコードを実際に起動し、エンドポイントが正しく動作することを確認する。
**⛔ スキップ禁止**（api Contract が 0 件でも確認スクリプトを実行する）。

```
実行内容（scripts/verify-web-app.sh が行う）:
1. 開発サーバーの起動
   - package.json の scripts から起動コマンドを検出（dev / start / serve 優先）
   - スクリプトが存在しない場合: framework に合わせた最小サーバー起動ファイル
     （src/server.ts など）と start スクリプトを自動生成してから起動する（スキップ禁止）
   - サーバーが起動するまで待機（最大 30 秒）

2. API エンドポイント動作確認
   - api タイプの Contract ごとに method + path にリクエストを送信
   - 5xx エラーが返った場合は失敗（exit 1）

3. 確認結果を .blueprint/reviews/web-verification-{timestamp}.md に保存

4. 開発サーバーの停止

5. フロントエンド確認（HTML を返すエンドポイントがある場合）
   - ブラウザでルートパスを開き、コンソールエラーを確認
```

**失敗時の扱い**:

```
- エンドポイント不応答 → サーバー起動ログを確認、原因を修正して再試行
- 5xx エラー → ログを確認、Integrator 相当の修正を行い再確認
- 修正できない場合 → ユーザーに報告してから Step 7 へ進む
  （Web 動作確認の失敗は Code Review Gate をブロックしない）
```

### Step 6.8: E2E テスト実行（条件付き）

条件: `.blueprint/config.yaml` の `tech_stack.frontend.e2e_tool` が `playwright` または `cypress`
**条件を満たさない場合はスキップしてStep 7へ進む。**

```
実行内容:
1. playwright.config.ts（または cypress.config.ts）の存在を確認
   - 存在しない場合: 「/test-from-contract を先に実行してください」と案内
2. E2E ランナーのブラウザをインストール（未インストール時）
...
4. 失敗した場合:
   - エラーログを確認して原因を分析
   - App.tsx の fetch wiring ミス / サーバー起動タイミング / テストデータの問題を確認
   - 修正して再実行（最大 3 回）
5. 3 回失敗した場合: ユーザーに報告して Step 7 へ進む
   （E2E 失敗は Code Review Gate をブロックしない）
```

### Step 7: 承認 + pipeline-state 更新

**⛔ 絶対必須: Code Review Gate（4 Agent 並列）を実行してから承認フェーズへ進む。テスト GREEN だけで完了とみなしてはならない。**

Code Review Gate 実行後は pipeline-state.yaml の `code_review_gate` を更新し、
`scripts/assert-gate-completed.sh` で実行済みを確認してからユーザーに承認サマリーを提示する。
...
stage_3_implement:
  ...
  web_verification:
    endpoints_checked: N
    failed_endpoints: []
    frontend_screenshot: true | false
  e2e:
    status: passed | failed | skipped      # e2e_tool が none の場合は skipped
    tests_passed: N
    tests_failed: N
...
tech_stack:
  ...
  frontend:                                             # screen Contract がある場合のみ生成
    framework: react | vue | svelte | next | none       # 検出 or 選択
    ui_library: shadcn | mui | antd | none              # 検出 or 選択
    test_tool: testing-library | playwright | none      # 検出（デフォルト: testing-library）
