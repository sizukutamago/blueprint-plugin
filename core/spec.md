...
#### user-stories.md からの入力（/requirements 実行済みの場合）

`docs/requirements/user-stories.md` が存在する場合、自動的に読み込んでスコープ確認のベースにする。

- Epic/Story 構造を Contract 一覧の初期候補として活用する
- US-XXX から FR-XXX を導出する（1 US = 1 FR、連番で自動採番）
- Contract の `links.implements` に FR-XXX を設定する
- `/requirements` で回答済みの内容（ターゲットユーザー、プラットフォーム、ビジネスルール等）はブレストで再質問しない
- user-stories.md が存在しない場合は、従来通りゼロからブレストを行う（互換性維持）

US-XXX → FR-XXX 導出ルール:
- 各 US-XXX に対して FR-XXX を 1:1 で採番（US-001 → FR-001, US-002 → FR-002）
- MoSCoW で Won't に分類された Story は FR を生成しない
- FR-XXX は Contract の links.implements で参照される
...
| 状況 | config.yaml 表記 |
|------|----------------|
| Drizzle ORM を使用 | `orm: drizzle` |
| Prisma を使用 | `orm: prisma` |
| TypeORM を使用 | `orm: typeorm` |
| ORM なし（SQL 直書き / 生 Driver） | `orm: none` |
| ORM なし（インメモリ / ファイル） | `orm: none` |

ORM を使用しない場合は `none` を明示する（フィールド省略は NG）。
`/implement` の技術スタック判断に使用するため、常に明示的な値が必要。
...
frontend セクションは screen Contract が 1 つ以上生成される場合に追加する。
API 専用と明示された場合のみスキップ（バックエンド専用プロジェクトに余分なフィールドを追加しない）。
...
- 入力のバリデーションルール（型、範囲、パターン）
  - **string フィールドは必ず確認**: min / max / nullable（空文字許容か）を明示
  - 例: 「description は省略できますか？ 省略時は空文字 / null のどちらですか？ 最大文字数は？」
- 異常系・エラーケース
- 状態遷移（あれば）
- 外部依存（外部 API、ファイル連携）
...
**screen Contract のデフォルト方針**:

フロントエンド（画面）あり が **デフォルト**。以下のキーワードが含まれる場合のみ「API のみですか？」と確認する:

| 確認が必要なキーワード | 例 |
|----------------------|-----|
| API / バックエンド / サーバー | 「API サーバーを作りたい」|
| SDK / ライブラリ / CLI | 「CLI ツールを作りたい」|
| バッチ / ジョブ / スクリプト | 「夜間バッチを作りたい」|

上記に該当しない場合（「アプリ」「サービス」「ツール」「システム」等）は **フロントエンドあり** と判断して screen Contract の設計に進む。
...
```
提案フォーマット:
| # | Contract ID | タイプ | 概要 | 依存先 | FR |
|---|------------|--------|------|--------|-----|
| 1 | CON-order-create | api | 注文作成 API | CON-stripe-payment-intent | FR-001 |
| 2 | CON-stripe-payment-intent | external | Stripe 決済 | — | FR-002 |
...
- ファイル名: `DEC-{NNN}-{kebab-case-slug}.md`（例: `DEC-001-done-immutable.md`）

**DEC ファイルを生成するタイミング**（Step 3 ブレスト中に随時記録）:

| ブレスト中の発言例 | DEC ファイルを生成する |
|-----------------|-------------------|
| 「done ステータスは一度設定したら変えられない」 | → DEC-001-done-immutable.md |
| 「外部 API は同期呼び出しにする」 | → DEC-00N-sync-external-api.md |
| 「削除は物理削除とする」 | → DEC-00N-physical-delete.md |
| 「パスワードは平文保存しない（当然）」 | → 記録しない（自明なため） |

> concepts/decisions はブレスト中に自然に出てくるもの。「何を作るか」ではなく「なぜそうするか」の記録。
> 「それは当然では？」と感じる決定でも、将来の担当者が迷う可能性があるなら記録する。
