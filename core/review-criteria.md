...
|--------|------|-----------|----------|
| **P0** | 要件不充足、根本的設計ミス | 要件定義 | 即差し戻し |
| **P1** | セクション間不整合（DB/API等） | 該当ステージ | 2件以上で差し戻し |
...
| **ROLLBACK_P1** | P0=0, P1>=2 | 該当ステージへ差し戻し |
| **ROLLBACK_P0** | P0>=1 | 要件定義へ即差し戻し |

## 差し戻しロジック

| 指摘カテゴリ | 重大度 | 差し戻し先 |
|-------------|--------|-----------|
| 未定義 ENT 参照 | P1 | 対象ステージ |
| API 参照切れ | P1 | 対象ステージ |
| 画面 ID 不整合 | P1 | 対象ステージ |
| NFR 未対応 | P1 | 対象ステージ |
| 技術スタック不整合（mode: specified 時） | P0 | 対象ステージ |
| 要件対応漏れ | P0 | 要件定義 |
| 形式エラー | P2 | 当該ステージ |
| 曖昧表現 | P2 | 当該ステージ |
